...
Source: project
Triggers: audit a skill, optimize a skill, skill quality review, skill best practices, fix skill triggering
...
**Round 0: Is this actually a skill?**
*Why this matters: Not every recurring workflow deserves a skill. Skills are suggestions the model considers each turn — they live in the routing budget. Some workflows belong in CLAUDE.md (always-applied rules), some belong in a hook (deterministic side effect), some belong in a one-shot script (called explicitly when needed). Picking the wrong primitive bloats the skill catalog and makes routing worse for every other skill.*

Ask:
- Is this a **rule** that should always apply (e.g., "never commit without running tests")? → CLAUDE.md, not a skill.
- Is this a **deterministic side effect** that should fire on a specific event (e.g., "after every commit, post to Slack")? → hook, not a skill.
- Is this a **one-off action** the user will invoke explicitly each time (e.g., "render this CSV as a chart")? → script + `run_skill_script`, optionally wrapped as a thin skill.
- Is this a **multi-step workflow** that needs the model's judgment to adapt to context? → skill, proceed to Round 1.

If the user pushes back ("I want it as a skill anyway"), respect that and proceed — but log the alternative in the final summary so they can reconsider later. The point of this round is to surface the decision, not block it.
...
**Step 4.5: Decide whether to delegate to sub-agents**

If the skill has any of these properties, consider delegating to the helpers in the `skill-helpers` skill:

- **Multi-domain work** (research + writing + chart-making, etc.) → `skill_decomposer` plans parallel fan-out so the primary agent doesn't serialize.
- **Subjective output the user reviews** (writing, design, plans, proposals) → `skill_critic` does a fresh-eyes pass before showing the draft to the user.
- **Ambiguous trigger surface** where the skill competes with adjacent skills → `skill_router` gives weaker primary models a focused routing decision.

Delegation matters most for low/mid-tier primary models (Haiku, 4o-mini, local). Frontier models will handle routing/critique/decomposition inline and should skip the helpers. Designing the skill so the helpers are *optional* — referenced in the body but not forced — gives you Minimum-Effective-Dose behavior across the whole tier range.

See `.claude/skills/skill-helpers/SKILL.md` for the prompt templates and call patterns.
