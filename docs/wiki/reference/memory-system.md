...
- **ResearchLM** — Optional fan-out to an external knowledge MCP when `RESEARCHLM_MCP_URL` and `RESEARCHLM_SEARCH_TOOL` are both set.
- **InsightsLM** — Optional fan-out when `INSIGHTSLM_MCP_URL` and `INSIGHTSLM_SEARCH_TOOL` are both set.
...
| `RESEARCHLM_MCP_URL` | — | Optional ResearchLM MCP server URL for retrieval fan-out. |
| `RESEARCHLM_SEARCH_TOOL` | — | Tool name to call on the ResearchLM server. |
| `INSIGHTSLM_MCP_URL` | — | Optional InsightsLM MCP server URL for retrieval fan-out. |
| `INSIGHTSLM_SEARCH_TOOL` | — | Tool name to call on the InsightsLM server. |
