...
NeuroClaw tracks models across three providers:
...
VoidAI is the only provider for which the catalog can discover new models at runtime. Anthropic and Codex do not expose public list endpoints, so those entries are kept current by re-seeding on each refresh cycle.
...
| POST | `/api/models/refresh` | Trigger immediate catalog refresh. Optional `?provider=voidai\|anthropic\|codex` |
