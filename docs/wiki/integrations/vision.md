...
The agent receives ordinary text — it does not need to support image inputs natively. The approach works with every provider NeuroClaw supports, including VoidAI, Anthropic, Codex, and Gemini.
...
| `native` | Pass the raw image through to the model directly. Only works reliably on OpenAI / VoidAI paths with a vision-capable model. Anthropic, Codex, and Gemini agents that resolve to `native` have attachments silently dropped with a logged warning — configure those agents as `preprocess` instead. |
...
- **Hermes / xAI (Grok)**: `grok-4.*` models are capable via the Hermes proxy.
- **Codex/Gemini CLI**: Always treated as not capable in headless mode. `auto` resolves to `preprocess`.
...
The vision service reuses the existing OpenAI-compatible client. By default it routes through the shared VoidAI client. When `VISION_PROVIDER` is set to `hermes`, calls are sent through the Hermes proxy client instead so you can use Grok/xAI models for image description.

You can override both the model and the provider with `VISION_MODEL` and `VISION_PROVIDER`.
