...
// Provider priority for global cross-provider triage. Lower number = preferred first.
// voidai leads (primary paid API), then anthropic (curated 3-model pool), then
// codex (ChatGPT subscription), then ollama (local), then everything else.
...
export interface ResolveOpts {
  ...
  global?:    boolean;    // search all providers; return resolvedProvider alongside model
}
 ...
 *
 * global=true (or no provider): cross-provider pool — picks the best chat-capable
 * model in the tier by provider priority (voidai > anthropic > codex > ollama).
 * The resolved provider is returned so callers (e.g. the spawner) can wire up
 * the correct API client.
 *
 * provider scoped: legacy behaviour — stays within the given provider.
 */
export function resolveModelForTier(tier: ModelTier, opts: ResolveOpts = {}): ModelResolution {
  const tierOrder: ModelTier[] = tier === 'high' ? ['high', 'mid', 'low']
                                : tier === 'mid'  ? ['mid', 'low', 'high']
                                :                   ['low', 'mid', 'high'];

  if (opts.global || !opts.provider) {
    // Cross-provider: query all configured providers, sort by priority.
    for (const t of tierOrder) {
      const rows = listCatalog({ tier: t, chatCapable: true });
      if (rows.length > 0) {
        rows.sort((a, b) => {
          const diff = providerPriority(a.provider) - providerPriority(b.provider);
          return diff !== 0 ? diff : a.model_id.localeCompare(b.model_id);
        });
        if (t !== tier) logger.warn('triage: tier empty in global pool, using fallback', { requested: tier, used: t, model: rows[0].model_id });
        return { model: rows[0].model_id, resolvedProvider: rows[0].provider };
      }
    }
    return { model: opts.preferred ?? null, resolvedProvider: 'voidai' };
  }

  // Provider-scoped resolution.
  const provider = opts.provider;
  for (const t of tierOrder) {
    const rows = listCatalog({ provider, tier: t, chatCapable: true });
    if (rows.length > 0) {
      if (t !== tier) logger.warn('triage: requested tier empty, using fallback', { requested: tier, used: t, model: rows[0].model_id, provider });
      return { model: rows[0].model_id, resolvedProvider: provider };
    }
  }
  return { model: opts.preferred ?? null, resolvedProvider: provider };
}
...
export interface PickModelOpts {
  ...
  global?:    boolean;    // cross-provider triage (spawner uses this)
}
...
export interface PickModelResult {
  ...
  resolvedProvider?: string;        // set when global=true; provider the model belongs to
}
...
export function pickModel(opts: PickModelOpts): PickModelResult {
  ...
  const resolution = resolveModelForTier(tier, { provider: opts.provider, preferred: opts.pinnedModel, global: opts.global });
  return { model: resolution.model, tier, triaged: opts.agentTier === 'auto', decision, depthPenalty, resolvedProvider: resolution.resolvedProvider };
}
...
export async function pickModelAsync(opts: PickModelOpts): Promise<PickModelResultExtended> {
  ...
  const resolution = resolveModelForTier(tier, { provider: opts.provider, preferred: opts.pinnedModel, global: opts.global });
  return { model: resolution.model, tier, triaged: true, decision, depthPenalty, llmEscalated, budgetDowngrade, resolvedProvider: resolution.resolvedProvider };
}
