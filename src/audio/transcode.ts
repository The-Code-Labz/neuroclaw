...
function inferExt(mimeType: string, filename?: string): string {
  // Prefer the MIME type over the filename: a mislabeled name (e.g. a WebM file
  // named `clip.ogg`, or a generic `blob`/`recording.dat`) must not override the
  // real container, or it would route into the wrong remux path (the ogg→webm
  // `-c:a copy` step can mangle a non-Opus stream).
  ...
  // MIME unknown/generic (e.g. application/octet-stream) — fall back to the
  // filename extension as a last resort.
