...
export const config = {
  ...
  get ncGit() {
    // Main-checkout coordination lock (spec 2026-07-16-main-checkout-coordination-lock).
    // Shadow-first: NC_GIT_ENFORCE defaults to 0 (log-only) so real worktree
    // traffic can bake before flipping to hard-block.
    return {
      repoRoot:       process.env.NC_GIT_REPO_ROOT?.trim() || '/home/neuroclaw-v1',
      enforce:        process.env.NC_GIT_ENFORCE === '1' || process.env.NC_GIT_ENFORCE?.toLowerCase() === 'true',
      lockDisabled:   process.env.NC_GIT_LOCK_DISABLED === '1' || process.env.NC_GIT_LOCK_DISABLED?.toLowerCase() === 'true',
      unsafe:         process.env.NC_GIT_UNSAFE === '1' || process.env.NC_GIT_UNSAFE?.toLowerCase() === 'true',
      lockWaitSeconds: parseInt(process.env.LOCK_WAIT_S ?? '60', 10),
    };
  },
  get studio() {
