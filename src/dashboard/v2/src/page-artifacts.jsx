            ...
            {entries?.map((e) => {
              ...
              return (
                <div key={`${e.session}/${e.agent}`} onClick={() => openWorkspace(e)}
                     style={{ padding: '6px 8px', marginBottom: 4, cursor: 'pointer', borderRadius: 2,
                ...
                {files.map((f) => {
                  ...
                  return (
                    <div key={f.relPath} onClick={() => openFile(f)}
                         style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 6px', cursor: 'pointer', borderRadius: 2,
                                  background: active ? 'rgba(255,255,255,0.05)' : 'transparent', borderBottom: '1px solid var(--line-soft)' }}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  {preview.file.kind === 'image' && (
                    <img src={rawUrl(preview.file)} alt={preview.file.name}
                         style={{ maxWidth: '100%', border: '1px solid var(--line-soft)', borderRadius: 2, background: '#0008' }}/>
                  )}
                  ...
                  {preview.file.kind === 'text' && (
                    preview.data === null
                      ...
                      : <>
                          ...
                          <pre className="mono" style={{ fontSize: 11, whiteSpace: 'pre-wrap', wordBreak: 'break-word', maxHeight: 480, overflowY: 'auto', margin: 0, padding: 8, background: 'rgba(0,0,0,0.3)', border: '1px solid var(--line-soft)', borderRadius: 2 }}>{preview.data.content}</pre>
                        </>
                  )}
