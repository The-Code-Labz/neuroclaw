          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div className="muted mono" style={{ fontSize: 10, marginTop: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {s.url}
                {s.last_probed_at && <span> · probed {new Date(s.last_probed_at).toLocaleTimeString('en-US', { timeZone: 'America/Los_Angeles', hour12: true })}</span>}
              </div>
...
const McpToolRow = ({ serverId, tool }) => {
  ...
  return (
    <div style={{ marginBottom: 6, paddingBottom: 6, borderBottom: '1px dashed color-mix(in srgb, var(--accent) 6%, transparent)' }}>
      ...
      {open && (
        <pre className="mono" style={{ fontSize: 10, background: 'var(--bg-1)', padding: 8, borderRadius: 3, marginTop: 6, maxHeight: 240, overflow: 'auto' }}>
