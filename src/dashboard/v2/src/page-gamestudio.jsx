...
// ─── Deploy-to-GitHub modal button ─────────────────────────────────────────
// Pushes the game to a GitHub repo via /api/studio/deploy-github (broker PAT,
// no local git). Optionally enables GitHub Pages for an instant playable URL.
          ...
          <div onClick={(e) => e.stopPropagation()} className="nc-card" style={{ width: 'min(440px,94vw)', padding: 18 }}>
            ...
            {!result ? (
              <>
                ...
                <input value={name} onChange={(e) => setName(e.target.value)} placeholder="my-game" disabled={busy}
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
                  ...
                  Enable GitHub Pages — instant playable URL (makes the repo public)
                </label>
            ...
            return (
              <div key={g.id} className="nc-card" style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <DeployButton
                    artifactId={g.artifacts?.[0]?.id}
                    defaultName={(g.brief?.brief || 'game').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 40)}
                    disabled={!playable} />
