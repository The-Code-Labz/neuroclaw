...
// 11-tab consolidated nav (v3 §1). Merged pages expose their old views as
// sub-tabs (SubTabs) inside the parent. Sessions folded into the Chat left rail.
const NAV = [
  { group: 'CORE', items: [
    { id: 'overview', label: 'Overview',        icon: 'overview' },
    { id: 'chat',     label: 'Chat',            icon: 'chat' },
    { id: 'agents',   label: 'Agents',          icon: 'agents' },
    { id: 'tasks',    label: 'Mission Control',  icon: 'tasks' },
    { id: 'studio',   label: 'Studio',          icon: 'canvas' },
  ]},
  { group: 'MIND', items: [
...
// Sessions are sourced from the live API (/api/sessions). The previous mock
// seed (s-1037..s-1042) caused a cold-load 404 because page-chat.jsx would
// auto-select SESSIONS[0].id and fetch /api/sessions/s-1042/messages before
// live-data.jsx replaced this array with real DB-backed UUIDs.
const SESSIONS = [];
...
const ANALYTICS = {
  ...
  providerSplit: [
    { name: 'Claude CLI', share: 0.55, color: 'var(--accent)' },
    { name: 'VoidAI', share: 0.28, color: 'var(--accent-2)' },
...
const STATUS = {
  ...
  tempAgents: 0,
  parallelSessions: 0,
  process: { memory: { rssMb: 0, heapUsedMb: 0, heapTotalMb: 0 }, cpuMicros: { user: 0, system: 0 }, cpuLoadPct: null },
  git: { branch: 'unknown', dirty: false, ahead: 0, behind: 0 },
};
...
const CORE = {
  ...
  uptimeSec: 0,
  checks: {
    router:     { state: 'warn', label: 'Router', detail: 'waiting for live data' },
    providers:  { state: 'warn', label: 'Providers', detail: 'waiting for live data' },
    agents:     { state: 'warn', label: 'Agents', detail: 'waiting for live data' },
    memory:     { state: 'warn', label: 'Memory', detail: 'waiting for live data' },
    mcp:        { state: 'warn', label: 'Tools', detail: 'waiting for live data' },
    background: { state: 'warn', label: 'Daemons', detail: 'waiting for live data' },
  },
  actions: {},
};

window.NC_DATA = { NAV, AGENTS, SESSIONS, TASKS, MEMORIES, HIVE_EVENTS, COMMS, COMMS_NOTES: [], NOTIFICATIONS: [], MCP_SERVERS, PROVIDERS, LOGS, ANALYTICS, DREAM, STATUS, LIVE_META, CORE, PROJECTS: [], SKILLS: [], ANALYST_ALERTS: [], HEALTH: null };
