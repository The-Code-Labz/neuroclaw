...
const Analytics = () => {
  ...
  const { msgs, topAgents, topTools, providerSplit, taskStats, c429, routingAccuracy, spawned, memoryWrites, memWritesHour } = ANALYTICS;
  const max = Math.max(...msgs);

  // heatmap 7x24 — real data from analytics_events
  const rawHeat = ANALYTICS.heatmap ?? [];
  const heatMap = {};
  rawHeat.forEach(h => { heatMap[`${h.day}-${h.hour}`] = h.count; });
  const heatMax = Math.max(1, ...rawHeat.map(h => h.count));
  const heat = Array.from({ length: 7 }, (_, d) =>
    Array.from({ length: 24 }, (_, h) => ((heatMap[`${d}-${h}`] ?? 0) / heatMax))
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ cursor: 'pointer' }} onClick={() => window.dispatchEvent(new CustomEvent('nc-goto', { detail: { page: 'health' } }))}>
          <StatCard label="ERRORS 24H ↗" value={ANALYTICS.logErrors24h ?? 0} sub="click → Health" tone={(ANALYTICS.logErrors24h ?? 0) > 0 ? 'red' : 'cyan'}/>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
        ...
        <Section title="TOP TOOLS">
          {topTools.map((t, i) => (
            <div key={i} style={{ marginBottom: 8 }}>
              ...
              <div className="bar-track"><div className="bar-fill" style={{ width: `${t.share*100}%`, background: 'linear-gradient(90deg, var(--accent-2), var(--violet))' }}/></div>
            </div>
          ))}
        </Section>
        <Section title="MEMORY WRITES · 7D">
          ...
          <div className="mono muted" style={{ fontSize: 10, marginTop: 8 }}>{memoryWrites} today · {memWritesHour} this hour</div>
        </Section>
      </div>
