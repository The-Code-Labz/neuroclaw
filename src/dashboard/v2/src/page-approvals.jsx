    ...
    <div>
      ...
      <div className="nc-panel glow" style={{ padding: 0, marginBottom: 16, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--line-soft)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {pending.length > 0 && (
            <div className="mono muted" style={{ fontSize: 10 }}>agents are waiting for your decision</div>
          )}
        </div>
            ...
            {pending.map((a) => {
              ...
              return (
                <div key={a.id} style={{ padding: '14px 16px', borderBottom: '1px dashed rgba(0,183,255,0.08)', display: 'grid', gridTemplateColumns: '80px 120px 100px 1fr auto', gap: 12, alignItems: 'center' }}>
                  ...
                  <span className="mono" style={{ fontSize: 11, color: 'var(--neon-2)' }}>@{a.agent_name || a.agent_id?.slice(0, 8) || '—'}</span>
