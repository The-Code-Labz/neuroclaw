...
const VideoCard = ({ item, ratio, onDownload, staggerIndex, isNew }) => {
  ...
  return (
    <div className="st-card" style={{ padding: 8, display: 'flex', flexDirection: 'column', gap: 8, position: 'relative', overflow: 'hidden', animation: animations || undefined }}>
...
const VideoGen = () => {
  ...
  return (
    <div className="page" style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
      ...
      {/* Current run */}
      {showGrid && (
        <div className="st-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', alignContent: 'start' }}>
      ...
      {history.length > 0 && (
        <>
          ...
          <div className="st-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', alignContent: 'start', opacity: 0.9 }}>
            {history.map((it, idx) => (
