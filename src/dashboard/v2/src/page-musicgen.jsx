...
const TrackCard = ({ item, onDownload, staggerIndex, isNew }) => {
  ...
  return (
    <div className="nc-panel glow tilt" style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 10, position: 'relative', overflow: 'hidden', animation: animations || undefined }}>
...
const TrackSkeleton = ({ elapsed }) => (
  <div className="nc-panel glow" style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
    <div style={{
    ...
    <div className="page" style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
      ...
      {history.length > 0 && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 12, alignContent: 'start', opacity: 0.9 }}>
            {history.map((it, idx) => (
