...
const Skills = () => {
  ...
  // Telemetry (Pillar 3 of MED skill plan) — passive, hint-only. Read once on
  // mount; we don't poll because the data only matters when an author is
  // looking at the catalog, not in real time.
  const [telemetry, setTelemetry] = React.useState({});    // skill name → { fire_count, last_fired_at, by_tier }

  React.useEffect(() => {
    let cancelled = false;
    fetch('/api/skills/telemetry', { credentials: 'same-origin' })
      .then(r => r.ok ? r.json() : { byName: {} })
      .then(j => { if (!cancelled) setTelemetry(j.byName || {}); })
      .catch(() => { /* telemetry is best-effort; UI still works without it */ });
        ...
        <div key={s.name} className="nc-panel" style={{ marginBottom: 10, padding: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {/* Telemetry line — Pillar 3. Passive. Shows fire count + last
                  used, or nothing if the skill has never fired. We render this
                  BEFORE the file path so it's eye-level with the description. */}
              {(() => {
                const t = telemetry[s.name];
                if (!t || !t.fire_count) {
                  return (
                    <div className="muted mono" style={{ fontSize: 10, marginTop: 4, opacity: 0.5 }}>
                      never fired
                    </div>
                  );
                }
                const last = relativeTimeFromNow(t.last_fired_at);
                const tierBits = Object.entries(t.by_tier || {})
                  .filter(([k]) => k && k !== 'unknown')
                  .map(([k, n]) => `${k}:${n}`).join(' · ');
                return (
                  <div className="muted mono" style={{ fontSize: 10, marginTop: 4 }}>
                    <span className="neonc">{t.fire_count}</span> injection{t.fire_count === 1 ? '' : 's'}
                    {' · last ' + last}
                    {tierBits ? ` · ${tierBits}` : ''}
                  </div>
                );
          ...
          {expanded[s.name] && (
            <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--border)' }}>
              ...
              <pre className="mono" style={{ fontSize: 11, background: 'var(--bg-1)', padding: 10, borderRadius: 3, maxHeight: 320, overflow: 'auto', whiteSpace: 'pre-wrap' }}>
...
// Compact relative-time formatter for the skill telemetry line. Goes from
// "just now" → "Nm ago" → "Nh ago" → "Nd ago". No deps, no Intl, no surprises.
...
const SkillEditor = ({ editing, setEditing, submit, cancel, busy }) => {
  // Collision check (Pillar 4) — only meaningful for the "new" and "edit" modes
  // where the user is authoring a SKILL.md description. Other modes (install,
  // upload, fromScript) bypass it because there's no description-being-typed
  // input surface for them. We debounce 400ms because typing fast shouldn't
  // hammer the endpoint, and we never block submission on the result — it's
  // pure information.
  const [collisions, setCollisions] = React.useState([]);
  const wantsCollisionCheck = editing.mode === 'new' || editing.mode === 'edit';
  React.useEffect(() => {
    if (!wantsCollisionCheck) { setCollisions([]); return; }
    const desc = (editing.description || '').trim();
    if (desc.length < 8) { setCollisions([]); return; }
    const handle = setTimeout(async () => {
      try {
        const r = await fetch('/api/skills/check-collision', {
          method: 'POST', credentials: 'same-origin',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
            name:        editing.name || undefined,    // exclude self in edit mode
            description: desc,
            threshold:   0.5,
            limit:       3,
          }),
        });
        if (!r.ok) { setCollisions([]); return; }
        const j = await r.json();
        setCollisions(Array.isArray(j.hits) ? j.hits : []);
      } catch {
        setCollisions([]);  // network hiccups silently degrade
      }
    }, 400);
    return () => clearTimeout(handle);
  }, [editing.description, editing.name, wantsCollisionCheck]);
  ...
  if (editing.mode === 'install') {
    ...
    return (
      <div className="nc-panel glow" style={{ padding: 16, marginBottom: 14 }}>
        ...
        {editing.output && (
          <pre className="mono" style={{ fontSize: 11, background: 'var(--bg-1)', padding: 10, borderRadius: 3, maxHeight: 360, overflow: 'auto', marginTop: 14, whiteSpace: 'pre-wrap' }}>
            {editing.output}
          </pre>
        )}
  ...
  return (
    <div className="nc-panel glow" style={{ padding: 16, marginBottom: 14 }}>
      ...
      <div className="field" style={{ marginBottom: 10 }}>
        ...
        {/* Collision hint — Pillar 4. Hint-only; nothing is blocked. Surfaces
            existing skills whose descriptions look similar to what's being typed,
            so the author can decide whether to merge or sharpen. */}
        {collisions.length > 0 && (
          <div className="mono" style={{ fontSize: 10, marginTop: 6, padding: '6px 8px', background: 'var(--bg-1)', borderLeft: '2px solid var(--warning, #d4a017)', borderRadius: 2 }}>
            <div className="muted" style={{ marginBottom: 3 }}>// description overlaps with existing skill{collisions.length === 1 ? '' : 's'} — sharpen, merge, or ignore:</div>
            {collisions.map(h => (
              <div key={h.name} style={{ display: 'flex', gap: 8 }}>
                <code className="neonc" style={{ fontSize: 10 }}>/{h.name}</code>
                <span className="muted" style={{ fontSize: 10 }}>({Math.round(h.similarity * 100)}%)</span>
                <span className="muted" style={{ fontSize: 10, opacity: 0.7, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{h.description}</span>
              </div>
            ))}
          </div>
        )}
