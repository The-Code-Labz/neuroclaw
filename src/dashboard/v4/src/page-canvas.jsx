/* Canvas v4 — Asia's design studio workspace.
 *
 * Implements the workspace surface from docs/specs/design-tab-ASAGI-brief.md.
 * Colors remapped to the v4 graphite/blue palette; layout keeps the three-column
 * split until the viewport is genuinely narrow.
 *
 *   ┌──────────────────────────────────────────────────────────────────┐
...
const BriefPanel = ({ form, setForm, onSubmit, busy }) => (
  <div className="nc-panel glow" style={{ padding: 14 }}>
    ...
    <textarea
      ...
      rows={4}
      style={{
        width: '100%', marginTop: 8, background: 'var(--surface-2)',
    ...
    <button onClick={onSubmit}
      ...
      style={{
        ...
        color: 'var(--text-inverted)', border: 0, borderRadius: 3,
...
const DirectionPicker = ({ directions, active, onPick }) => (
  <div className="nc-panel" style={{ padding: 12, marginTop: 12 }}>
    <div className="label-tiny" style={{ color: 'var(--accent)' }}>// DIRECTION</div>
      ...
      {directions.map(d => (
        <button key={d.id}
          ...
          style={{
            textAlign: 'left',
            padding: '8px 10px',
            background: active === d.id ? 'var(--accent-soft)' : 'var(--surface-2)',
            border: '1px solid ' + (active === d.id ? 'var(--accent)' : 'var(--border-default)'),
...
const LivePlanCard = ({ todos, status, errors }) => (
  <div className="nc-panel" style={{ padding: 12, marginTop: 12 }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <div className="label-tiny" style={{ color: 'var(--accent)' }}>// LIVE PLAN</div>
      <span className={`tag ${status === 'complete' ? 'green' : status === 'error' ? 'red' : 'accent'}`} style={{ fontSize: 9 }}>
        {status || 'idle'}
      </span>
    </div>
      ...
      {todos.map(t => (
        <div key={t.id} className="mono" style={{ display: 'flex', gap: 8, fontSize: 11, alignItems: 'center' }}>
          <span style={{
            ...
            background:
              t.status === 'completed'   ? 'var(--accent)' :
              t.status === 'in_progress' ? 'var(--warning)'  : 'transparent',
          }} />
          <span style={{ color: t.status === 'completed' ? 'var(--text-tertiary)' : 'var(--text-primary)', textDecoration: t.status === 'completed' ? 'line-through' : 'none' }}>
            {t.text}
          </span>
        </div>
      ))}
    ...
    {errors.length > 0 && (
      <div style={{ marginTop: 10, padding: 8, background: 'rgba(242, 139, 130, 0.08)', border: '1px solid var(--error)', borderRadius: 3 }}>
        {errors.map((e, i) => <div key={i} className="mono" style={{ fontSize: 10, color: 'var(--error)' }}>! {e}</div>)}
      </div>
    )}
  </div>
);
  ...
  return (
    <div style={{ position: 'relative', flex: 1, display: 'flex' }}>
      ...
      <iframe
        ...
        style={{ width: '100%', flex: 1, border: '1px solid var(--border-default)', background: '#fff', borderRadius: 3, minHeight: 720 }}
      />
    </div>
  );
};
...
const ExpandedOverlay = ({ artifact, onClose }) => {
  ...
  return (
    <div style={{
      ...
      background: 'rgba(3, 4, 7, 0.92)', display: 'flex', flexDirection: 'column', padding: 16,
    }}>
      ...
      {artifact.type === 'html' ? (
        <iframe
          ...
          srcDoc={withCspMeta(artifact.content)}
          sandbox="allow-scripts"
          style={{ flex: 1, width: '100%', border: '1px solid var(--border-default)', background: '#fff', borderRadius: 3 }}
        />
      ) : (
  ...
  return (
    <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
      <svg width={180} height={180} style={{ flexShrink: 0 }}>
        ...
        {dims.map((d, i) => {
          const [x, y] = pt(i, 10);
          return <line key={d} x1={cx} y1={cy} x2={x} y2={y} stroke="var(--border-default)" strokeWidth="1" />;
        })}
        <polygon points={poly} fill="var(--accent-soft)" stroke="var(--accent)" strokeWidth="1.5" />
        {dims.map((d, i) => {
          const [x, y] = pt(i, 11);
          return <text key={d} x={x} y={y} fontSize="9" fill="var(--text-secondary)" fontFamily="var(--mono)" textAnchor="middle">{labels[d]}</text>;
        })}
      </svg>
      <div style={{ flex: 1 }}>
        <div className="label-tiny" style={{ color: 'var(--accent)' }}>// NOTES</div>
        <ul className="mono" style={{ fontSize: 10, color: 'var(--text-secondary)', marginTop: 6, paddingLeft: 16, lineHeight: 1.5 }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ maxHeight: 120, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
        {messages.map((m, i) => (
          <div key={i} className="mono" style={{
            ...
            borderLeft: `2px solid ${m.role === 'user' ? 'var(--accent)' : 'var(--text-tertiary)'}`,
          }}>
            <span style={{ color: 'var(--text-tertiary)' }}>{m.role}:</span> {m.text}
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 6 }}>
        <input
          ...
          disabled={busy}
          style={{
            flex: 1, padding: '6px 8px', background: 'var(--surface-2)',
            color: 'var(--text-primary)', border: '1px solid var(--border-default)',
...
const ExportBar = ({ artifact, onMultiCritique, multiBusy }) => {
  ...
  return (
    <div>
      <div className="label-tiny" style={{ color: 'var(--accent)' }}>// EXPORT</div>
      ...
      {err && <div className="mono" style={{ fontSize: 10, color: 'var(--error)', marginTop: 6 }}>! {err}</div>}
    </div>
  );
};
...
const ProjectHistory = ({ projects, activeId, onPick, onDelete }) => (
  <div>
    <div className="label-tiny" style={{ color: 'var(--text-tertiary)' }}>// HISTORY</div>
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 8 }}>
      ...
      {projects.slice(0, 12).map(p => (
        <div key={p.id} style={{
          ...
          border: '1px solid ' + (activeId === p.id ? 'var(--border-strong)' : 'var(--border-subtle)'),
          borderRadius: 3, padding: '4px 6px',
        }}>
          <button onClick={() => onPick(p.id)} style={{ flex: 1, background: 'transparent', border: 0, textAlign: 'left', cursor: 'pointer', color: 'var(--text-primary)' }}>
...
const CanvasFooter = () => (
  <div className="mono muted" style={{ fontSize: 9, padding: '8px 0', textAlign: 'right', borderTop: '1px dashed var(--border-subtle)', marginTop: 12 }}>
...
const ActivityLog = ({ entries, open, onToggle }) => {
  ...
  const colorOf = (k) => k === 'error' ? 'var(--error)' : k === 'ok' ? 'var(--accent)' : 'var(--text-secondary)';
  return (
    <div className="nc-panel" style={{ marginTop: 12, padding: 0, overflow: 'hidden' }}>
      ...
      {open && (
        <div style={{ background: 'var(--bg-base)', padding: '8px 12px', maxHeight: 220, overflow: 'auto' }}>
          {entries.length === 0 && (
  ...
  return (
    <div className="page" style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
      <PageHeader
        ...
        right={
          <div style={{ display: 'flex', gap: 6 }}>
            <span className="tag accent" style={{ fontSize: 10 }}>POWERED BY ASIA</span>
