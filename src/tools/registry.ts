...
export const registry: ToolDef[] = [
  ...
  {
    name:        'webapp_build',
    description: 'Build a genuinely functional modern web app from a description ("lovable/bolt"-style). Produces ONE self-contained HTML app (React + Tailwind via CDN, real state + interactivity) that lands in Studio › WebApps and previews in a sandboxed iframe. Reuses the Canvas engine with a web-app prompt; pick ANY registered agent via agent_name to build it (not every deployment has a "Jarvis"). Runs in a sandboxed iframe (no cookies/storage) but may fetch public HTTPS APIs. Returns the app id + a preview URL. Takes ~30s–3 min.',
    schema:  S.webappBuildSchema,
    shape:   S.webappBuildShape,
    handler: async (args) => {
      const brief = (args.brief ?? '').trim();
      if (!brief) return { ok: false, error: 'brief is required' };
      const agentName = typeof args.agent_name === 'string' && args.agent_name.trim() ? args.agent_name.trim() : undefined;
      const { generate } = await import('../skills/canvas');
      let projectId: string | undefined;
      let artifactId: string | undefined;
      let error: string | undefined;
      let chars = 0;
      try {
        for await (const evt of generate({ brief, kind: 'webapp', agentName })) {
          if (evt.type === 'project.start') projectId = evt.payload.projectId;
          else if (evt.type === 'chunk') chars += (evt.payload?.text || '').length;
          else if (evt.type === 'artifact.emit') artifactId = evt.payload.id;
          else if (evt.type === 'error') error = evt.payload?.message;
      ...
      if (error) return { ok: false, error };
      if (!artifactId) return { ok: false, error: `webapp build produced no artifact (${chars} chars streamed)` };
      logger.info(`webapp_build: built app — project=${projectId} artifact=${artifactId} agent=${agentName || 'default'} chars=${chars}`);
      return {
        ok: true,
        webapp_id: projectId,
        artifact_id: artifactId,
        preview_url: `/api/webapp/artifact/${artifactId}/file`,
        built_by: agentName || 'default',
        note: 'Previewable in Studio › WebApps. Sandboxed (no cookies/storage); may call public HTTPS APIs.',
      };
    },
  },

  // ── Deploy a Studio artifact to GitHub ─────────────────────────────────────
  {
    name:        'deploy_to_github',
    description: 'Deploy a Studio artifact (design / game / web app) to a GitHub repository via the GitHub REST API — creates or updates a repo and commits the artifact as index.html, optionally enabling GitHub Pages so it is instantly live at a public URL. Uses the shared GitHub token (no local git). The artifact_id must be an HTML artifact from a canvas/game/webapp build. Returns the repo URL (+ Pages URL if enabled).',
    schema:  S.deployToGithubSchema,
    shape:   S.deployToGithubShape,
    handler: async (args) => {
      ...
      if (!repoName)   return { ok: false, error: 'repo_name is required' };
      const { getArtifact, deployHtmlToGithub } = await import('../skills/canvas');
      const a = getArtifact(artifactId);
      if (!a) return { ok: false, error: `artifact not found: ${artifactId}` };
      if (a.type !== 'html') return { ok: false, error: 'only HTML artifacts can be deployed to GitHub' };
      try {
        const r = await deployHtmlToGithub({
          html: a.content, repoName, title: a.title,
          isPrivate: args.private !== false,
          enablePages: !!args.enable_pages,
        });
        return {
          ok: true,
          repo_url: r.repoUrl,
          pages_url: r.pagesUrl,
          repo: `${r.owner}/${r.repo}`,
          created: r.created,
          note: r.pagesUrl ? 'Live via GitHub Pages (may take ~1 min to build).' : 'Committed to the repo.',
        };
      } catch (err) {
        return { ok: false, error: (err as Error).message };
      }
