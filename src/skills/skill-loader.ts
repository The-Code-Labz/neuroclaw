...
//
// Walks supported skill roots, parses YAML frontmatter, exposes:
...
//
// Skill roots are searched, in order:
//   1. <project>/.claude/skills/    — project-local skills (checked into the repo)
//   2. ~/.claude/skills/            — user-global skills (Claude CLI default)
//   3. <project>/.agents/skills/    — installed skills from cross-agent packs
//   4. Claude plugin / marketplace skill bundles
...
export type SkillSource = 'project' | 'user' | 'installed' | 'plugin' | 'marketplace';

const PROJECT_ROOT  = path.resolve(process.cwd(), '.claude/skills');
const USER_ROOT     = path.join(os.homedir(), '.claude', 'skills');
const INSTALLED_ROOT = path.resolve(process.cwd(), '.agents/skills');
...
function readSkillFile(absPath: string, source: SkillSource, plugin?: string): SkillRecord | null {
  try {
...
function walkSkills(root: string, source: 'project' | 'user' | 'installed', acc: Map<string, SkillRecord>): void {
...
function refreshCache(): Map<string, SkillRecord> {
  ...
  //   project > user > installed > installed-plugin > marketplace
  ...
  walkSkills(INSTALLED_ROOT, 'installed', map);
 ...
 *
 * Minimum-Effective-Dose (MED) tier shaping — `opts.tier` controls how much
 * scaffolding wraps each skill body:
 *
 *   • 'pinned' | 'auto' | 'high' | undefined → no scaffolding. Frontier models
 *     (Opus, Sonnet 4.5, GPT-5, Gemini 2.5 Pro, …) read raw skill bodies and
 *     route on their own. Anything extra is noise that crowds reasoning.
 *
 *   • 'mid' → light routing hint: each skill gets a one-line "**Use when:**"
 *     header derived from its description. Helps mid-tier models (Sonnet 3.5,
 *     GPT-4.1, Haiku 3.5) pick the right skill without over-constraining.
 *
 *   • 'low' → explicit routing scaffolding: prepends a one-paragraph router
 *     instruction telling the model to match user intent against the
 *     **Triggers** list and follow the matched skill's steps verbatim. Designed
 *     for Haiku / 4o-mini / local 8B models that need hand-holding.
 *
 * Backward-compatible: callers passing only `names` get the legacy (frontier)
 * behavior unchanged.
 */
export interface BuildSkillsBlockOptions {
  /** Agent's model tier. See module docstring for behavior per tier. */
  tier?: 'pinned' | 'auto' | 'low' | 'mid' | 'high' | string | null | undefined;
}

export function buildSkillsBlock(
  names: string[] | null | undefined,
  opts: BuildSkillsBlockOptions = {},
): string {
  ...
  // Normalize tier. Anything we don't recognize is treated as frontier — we'd
  // rather be invisible than wrong.
  const tier = (opts.tier ?? 'pinned').toString().toLowerCase();
  const lowTier = tier === 'low';
  const midTier = tier === 'mid';

  const lines: string[] = ['', '---', '## Active skills'];

  // Low-tier router preamble. One short paragraph — anything longer becomes
  // noise that small models start narrating instead of acting on.
  if (lowTier && found.length > 0) {
    lines.push('');
    lines.push(
      '> **Skill router:** before answering, scan the **Triggers** of each skill below. ' +
      'If the user\'s request matches one, follow that skill\'s steps verbatim. ' +
      'If two skills could match, pick the one with the more specific trigger. ' +
      'If none match, answer normally without invoking a skill.',
    );
  }
  ...
  for (const s of found) {
    ...
    // Mid/low tiers get a "Use when" line above the italic description so the
    // router signal is visually distinct from the descriptive text.
    if ((midTier || lowTier) && s.description) {
      lines.push(`**Use when:** ${s.description}`);
 ...
 *  Enforces the declared scripts[] allowlist when the skill has one. */
export function getSkillScriptPath(skillName: string, filename: string): string {
  ...
  // Enforce the frontmatter scripts[] allowlist — only declared scripts may run.
  // When the list is empty the skill hasn't declared any (legacy fallback path);
  // in that case any file in scripts/ is accepted (same as the old behaviour).
  if (skill.scripts.length > 0 && !skill.scripts.includes(fname)) {
    throw new Error(`script "${fname}" is not declared in skill "${skillName}"'s scripts list`);
  }
  const scriptsDir = path.join(skill.dir, 'scripts');
  const target = path.resolve(scriptsDir, fname);
  // Defense in depth — even though validateScriptFilename rejects path components,
  // double-check the resolved path stays strictly inside scriptsDir.
  if (!target.startsWith(path.resolve(scriptsDir) + path.sep)) {
...
/** Same as resolveEffectiveSkillNames, but also returns whether each name came
 *  from the agent's declared list or was forced in via always_on. The caller
 *  uses this to write accurate per-injection telemetry. */
export interface EffectiveSkill {
  name:   string;
  source: 'declared' | 'always_on';
}

export function resolveEffectiveSkillsDetailed(declared: string[]): EffectiveSkill[] {
  const declaredSet = new Set(declared);
  const out: EffectiveSkill[] = declared.map(n => ({ name: n, source: 'declared' as const }));
  for (const n of getAlwaysOnSkillNames()) {
    if (!declaredSet.has(n)) {
      declaredSet.add(n);
      out.push({ name: n, source: 'always_on' as const });
    }
  }
  return out;
}
