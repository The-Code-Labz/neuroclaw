...
`setup.sh` checks your Node version (>= 20), runs `npm install`, creates `.env`
from the example, **auto-generates a strong `DASHBOARD_TOKEN`**, builds the
dashboard, and prints how to start. Then add your `VOIDAI_API_KEY` to `.env`
(the primary provider) and run `npm run dashboard`.
...
cp .env.example .env
# Edit .env — set VOIDAI_API_KEY and a strong DASHBOARD_TOKEN
...
Then open **http://localhost:3141/** and sign in with your `DASHBOARD_TOKEN`
on the login page. The token is stored in a secure HttpOnly cookie — it never
appears in the URL. (View your token any time with `grep DASHBOARD_TOKEN .env`.)

---

## Running as a System Service (systemd)

The dashboard can be installed as a systemd service so it starts automatically on boot and restarts on crash.

### Install

```bash
# Service file is already at /etc/systemd/system/neuroclaw-dashboard.service
# Enable and start it:
systemctl daemon-reload
systemctl enable neuroclaw-dashboard
systemctl start neuroclaw-dashboard
```
...
```bash
# Check status
systemctl status neuroclaw-dashboard

# View live logs
journalctl -u neuroclaw-dashboard -f

# View last 50 lines of logs
journalctl -u neuroclaw-dashboard -n 50 --no-pager

# Restart the service
systemctl restart neuroclaw-dashboard

# Stop the service
systemctl stop neuroclaw-dashboard

# Disable auto-start on boot
systemctl disable neuroclaw-dashboard
```

### Notes

- The service runs as root from `/home/neuroclaw-v1` and loads `.env` automatically.
- Port 3141 is cleared before each start (`ExecStartPre`) to avoid conflicts with any stale processes.
- `Restart=always` with a 3-second delay means the dashboard survives crashes and comes back automatically.
- To update environment variables, edit `.env` then run `systemctl restart neuroclaw-dashboard`.
...
| `MCP_ENABLED` | No | `false` | Master switch for MCP integrations (NeuroVault) |
...
Helpers: `indexMemory()`, `searchMemoryIndex()`, `listMemoryIndex()`, `touchMemoryAccess()`, `attachVaultNote()` in `src/memory/memory-service.ts`. The retriever (`src/memory/memory-retriever.ts`) currently runs SQLite-only; vault fan-out is scheduled for P2.
...
RETRIEVER (fan-out)    →  SQLite memory_index ∪ Vault MCP, ranked
...
| `retrieve() fan-out` | Merged ranking across SQLite + Vault MCP |
...
- **Retriever fan-out** — SQLite + Vault MCP merged, dedup'd, ranked by `salience × decay × 0.6 + importance × 0.4` (14d half-life decay).
